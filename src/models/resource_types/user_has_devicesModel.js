...
const getUsersHasDevicesModel = ({conn, ...rest}) => {
    ...
    return mysql    
        ...
        .then(results => results.map(({id, fk_user, fk_device, created, deleted, createdBy, deletedBy, ...resultFiltered}) => resultFiltered))
}
...
const insertUsersHasDevicesModel = ({conn, ...rest}) => {
    ...
    return mysql
        ...
        .then(results => results[1].map(({id, fk_user, fk_device, created, deleted, createdBy, deletedBy, ...resultFiltered}) => resultFiltered))
}
