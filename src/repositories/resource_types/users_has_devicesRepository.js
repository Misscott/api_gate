...
const insertUsersHasDevicesQuery = () =>{
    return `
        ...
        LEFT JOIN 
            mydb.users as users ON ud.fk_user = users.id
        LEFT JOIN 
            mydb.devices as devices ON ud.fk_device = devices.id
        WHERE 
            ud.uuid = :uuid;
    `
};
 ...
 */
const modifyUsersHasDevicesQuery = ({ stock, new_user_uuid, new_device_uuid, price, isForSale }) => {
    const setClause = [
      new_device_uuid && 'fk_device = (SELECT id FROM mydb.devices WHERE uuid = :new_device_uuid)',
      new_user_uuid && 'fk_user = (SELECT id FROM mydb.users WHERE uuid = :new_user_uuid)',
      stock !== undefined && 'stock = :stock',
      price !== undefined && 'price = :price',
      isForSale !== undefined && 'isForSale = :isForSale'
    ].filter(Boolean).join(',\n    ');
  
    const showNewUserCondition = new_user_uuid
      ? 'AND ud.fk_user = (SELECT id FROM mydb.users WHERE uuid = :new_user_uuid)'
      : 'AND ud.fk_user = (SELECT id FROM mydb.users WHERE uuid = :user_uuid)';
  
    const showNewDeviceCondition = new_device_uuid
      ? 'AND ud.fk_device = (SELECT id FROM mydb.devices WHERE uuid = :new_device_uuid)'
      : 'AND ud.fk_device = (SELECT id FROM mydb.devices WHERE uuid = :device_uuid)';
    ...
    return `
      ...
      WHERE
          ...
          AND users_has_devices.fk_device = (SELECT id FROM mydb.devices WHERE uuid = :device_uuid)
          AND deleted IS NULL;
